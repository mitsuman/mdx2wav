...
//	初期化
//
bool OPM::Init(uint c, uint rf, bool f)
{
	if (!SetRate(c, rf, f))
		return false;
...
//	再設定
//
bool OPM::SetRate(uint c, uint r, bool f)
{
	clock = c;
	rate = r;
	filter = f;
...
//#define IStoSample(s)	((s * fmvolume) >> 14)
	...
	if (activech & 0x5555)
	{
		...
		for (Sample* dest = buffer; dest < limit; dest+=2)
		{
			...
			if (filter)
			{
				OpmHpfInp[0] = IStoSample(ibuf[1] + ibuf[3])<<14;
				OpmHpfInp[1] = IStoSample(ibuf[2] + ibuf[3])<<14;
				...
				StoreSample(dest[0], InpOpm[0] >> 5);
				StoreSample(dest[1], InpOpm[1] >> 5);
			}
			else
			{
